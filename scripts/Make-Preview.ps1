...
foreach ($name in @('easy','medium','hard')) {
    $variants[$name] = Get-Content ('dist/cinis-four-directions/data/phil/cinis_four_v2/' + $name + '/cinis_four_v2_' + $name + '.a7tinfo.layout.json') -Raw | ConvertFrom-Json
}
...
<h1>Vier Cinis um Latium</h1><p>Layout für neue Spiele (Version 0.3.0, <code>cinis_four_v2</code>) · 4096 × 4096 · Spielansicht um 45° gedreht</p>
...
<p>Rechtecke zeigen maximale Inselabmessungen, keine Küstenkonturen. Ein Mouseover zeigt Position, Größe und bei den Cinis die Drehung (<code>Rotation90</code>). Der äußere Rahmen ist der spielbare Bereich 268…3828. Normale Inseln werden weiterhin vom Spiel ausgewählt.</p>
<p class="warning">Prototyp: Die Vorschau zeigt nur die Vorlagendaten und belegt keine Vulkanmechanik.</p>
<script>
...
function draw(){svg.replaceChildren();box(268,268,3560,'#173543','#527989');box(1044,1044,2000,'#1a4149','#448477');for(const slot of layouts[document.querySelector('#variant').value]){let e;if(slot.kind==='Start'){const[x,y]=project(slot.x,slot.y);e=add('circle',{cx:x,cy:y,r:6,fill:'#ffe09a'})}else{e=box(slot.x,slot.y,slot.size,slot.kind==='Cinis'?'#ec9d58':'#59ad9a','#102330');e.setAttribute('fill-opacity',slot.kind==='Cinis'?'.9':'.65')}const title=document.createElementNS(ns,'title');title.textContent=`${slot.kind} ${slot.label}: (${slot.x}, ${slot.y}), ${slot.size} × ${slot.size}${slot.rotation90===undefined?'':', Rotation90 '+slot.rotation90}`;e.appendChild(title)}add('text',{x:520,y:45},'NORDEN · CINIS');add('text',{x:940,y:510},'OSTEN');add('text',{x:520,y:985},'SÜDEN · CINIS');add('text',{x:90,y:510},'WESTEN');add('text',{x:520,y:470,'font-weight':'bold'},'LATIUM')}
